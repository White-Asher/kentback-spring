...
## 2026-02-16 - Phase 6.3 Coupon Query

### Green
- 구현: `CouponLifecycleService.listAvailableCoupons(...)` 추가
- 구현: `CouponLifecycleServiceTest`에 사용 가능 쿠폰 조회 3개 테스트 추가
- 실행(단위): `.\gradlew test --tests "example.com.kentbackspring.coupon.CouponLifecycleServiceTest"`
- 결과(단위): 성공 (`BUILD SUCCESSFUL`)
...
## 2026-02-16 - Phase 7.2 Order Cancel (Partial Cancel - Product Coupon)

### Green
- 구현: `ProductCouponOrder`, `ProductCouponOrderItem`, `ProductCouponCancelService`, `PartialCancelResult` 추가
- 구현: `ProductCouponCancelServiceTest`에 부분 취소 3개 테스트 추가
- 검증: 쿠폰 적용 상품 취소 시 복구/비적용 상품 취소 시 유지/환불 금액 계산 확인
- 실행(단위): `.\gradlew test --tests "example.com.kentbackspring.coupon.ProductCouponCancelServiceTest"`
- 결과(단위): 성공 (`BUILD SUCCESSFUL`)
