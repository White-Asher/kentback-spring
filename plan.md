...
- [x] 하나의 주문에 하나의 쿠폰만 적용할 수 있다
- [x] 여러 쿠폰을 동시에 적용하려고 하면 예외가 발생한다
- [x] 사용자가 적용할 쿠폰을 명시적으로 선택할 수 있다

### 5.2 자동 최대 할인 선택

- [x] 여러 쿠폰 중 할인 금액이 가장 큰 쿠폰을 자동 선택한다
- [x] 정액 할인 금액이 같으면 먼저 만료되는 쿠폰을 선택한다

### 5.3 순차 적용 정책 (선택적 구현)

- [x] 상품 쿠폰 적용 후 장바구니 쿠폰을 순차 적용할 수 있다
- [x] 순차 적용 시 할인 순서를 명확히 정의한다
- [x] 순차 적용 결과가 중복 불가 정책보다 유리한지 비교한다
...
- [x] 사용자에게 쿠폰을 발급할 수 있다
- [x] 발급된 쿠폰은 미사용 상태로 시작한다
- [x] 이미 발급된 쿠폰은 다시 발급할 수 없다
- [x] 발급 시 발급 일시가 기록된다

### 6.2 쿠폰 사용

- [x] 미사용 쿠폰을 사용 처리할 수 있다
- [x] 사용된 쿠폰은 사용 일시가 기록된다
- [x] 이미 사용된 쿠폰은 다시 사용할 수 없다
- [x] 쿠폰 사용 시 주문 ID가 연결된다

### 6.3 쿠폰 조회

- [x] 사용자의 사용 가능한 쿠폰 목록을 조회할 수 있다
- [x] 만료된 쿠폰은 사용 가능 목록에서 제외된다
- [x] 이미 사용된 쿠폰은 사용 가능 목록에서 제외된다
...
- [x] 주문 전체 취소 시 사용된 쿠폰이 복구된다
- [x] 복구된 쿠폰은 미사용 상태로 변경된다
- [x] 쿠폰 사용 이력은 감사 목적으로 유지된다
- [x] 만료된 쿠폰의 유효기간 연장 정책을 적용할 수 있다

### 7.2 부분 취소 - 상품 쿠폰

- [x] 쿠폰이 적용된 상품만 취소 시 쿠폰이 복구된다
- [x] 쿠폰이 적용되지 않은 상품만 취소 시 쿠폰은 유지된다
- [x] 부분 취소 시 환불 금액이 정확히 계산된다

### 7.3 부분 취소 - 장바구니 쿠폰

- [x] 상품 부분 취소 시 해당 상품에 분배된 할인 금액이 환불된다
- [x] 부분 취소 후 남은 금액이 최소 주문 금액 미달이어도 쿠폰은 유지된다
- [x] 부분 취소 후 환불 금액과 남은 주문 금액의 합이 원래 금액과 일치한다
...
- [x] 한정 수량 쿠폰의 동시 발급 요청을 올바르게 처리한다
- [x] 발급 가능 수량을 초과하는 발급 요청은 실패한다
- [x] 동시성 상황에서 중복 발급이 발생하지 않는다

### 8.2 쿠폰 사용 제어

- [x] 동일 쿠폰의 동시 사용 요청은 하나만 성공한다
- [x] 이미 사용 중인 쿠폰은 다른 주문에서 사용할 수 없다
